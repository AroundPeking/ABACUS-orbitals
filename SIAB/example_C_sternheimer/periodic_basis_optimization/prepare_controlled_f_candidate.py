...
def _radial_diagnostics(radius: np.ndarray, radial: np.ndarray) -> dict:
    ...
    first_root = float(spherical_bessel_roots(3, RADIAL_ROWS)[0])
    return {
        "angular_momentum": 3,
        "first_bessel_root": first_root,
        "interior_node_count": _count_interior_nodes(radial),
        "kinetic_energy_hartree": 0.5 * (first_root / RCUT_BOHR) ** 2,
        "kinetic_energy_ry": (first_root / RCUT_BOHR) ** 2,
...
def prepare_candidate(*, source: Path, root: Path) -> dict:
    ...
    diagnostics = _radial_diagnostics(radius, orbitals[3][:, 0])
    ...
    try:
        coefficient_name = "C_3s3p2d1f_lowest_l3_bessel.txt"
        orbital_name = "C_gga_10au_100Ry_3s3p2d1f_lowest_l3_bessel.orb"
...
def main() -> None:
    ...
    result = prepare_candidate(source=args.source, root=args.root)
