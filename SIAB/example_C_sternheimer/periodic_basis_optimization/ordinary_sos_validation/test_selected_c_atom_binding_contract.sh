...
grep -q '^work=\$root/producer-retry1$' "$producer"
...
grep -q 'set_input_key sternheimer_mpi_layout frequency_grouped' "$producer"
grep -q 'remove_input_key sternheimer_frequency_grid_file' "$producer"
...
grep -q '^producer=\$base/selected-c-atom-response-aware-product-pca-55d25e3c9/producer-retry1$' "$sos"
grep -q 'task = rpa' "$sos"
grep -q 'nfreq = 6' "$sos"
...
grep -q '^producer=\$atom_root/producer-retry1$' "$delta"
...
grep -q '^producer=\$atom_root/producer-retry1$' "$reader"
