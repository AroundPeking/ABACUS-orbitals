...
test -s "$response_script" && test -s "$consumer_script"
...
canonical_csv=$(IFS=,; echo "${representatives[*]}")
{
  ...
  sha256sum "$response_script" "$consumer_script" "$frequency" "$sos_summary" "$report"
} > "$pending"
...
mv "$pending" "$chain"
echo "C24_RESPONSE_PCA_MATCHED_DELTA_RELEASE_OK response_job=$response_job consumer_job=$consumer_job"
