...
set -euo pipefail
trap 'status=$?; echo "C24_SELECTED_MATCHED_DELTA_RELEASE_FAILED line=$LINENO command=$BASH_COMMAND status=$status" >&2; exit $status' ERR
...
grep -qx "selected_sos_job=$selected_sos_job" "$q1/RUN_PROVENANCE.txt"
grep -qx "grid_coulomb_array_job=$grid_job" "$q1/RUN_PROVENANCE.txt"
test ! -e "$chain"
test ! -e "$pending"
mkdir "$lock"
...
seconds = parts[-1] + 60.0 * parts[-2]
if len(parts) == 3:
    seconds += 3600.0 * parts[0]
...
sbatch --test-only --array="$remaining_csv%1" \
  ...
  "$response_script" > "$root/REMAINING_Q_TEST_ONLY.txt"
...
sbatch --test-only \
  ...
  "$consumer_script" > "$root/CONSUMER_TEST_ONLY.txt"
