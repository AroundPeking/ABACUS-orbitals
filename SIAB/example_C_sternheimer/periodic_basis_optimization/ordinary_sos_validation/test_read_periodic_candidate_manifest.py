...
class ReadPeriodicCandidateManifestTest(unittest.TestCase):
    ...
    def test_reads_controlled_contracted_f_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "controlled-contracted-f.orb"
            orbital.write_text("orbital\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_contracted_f", "nu": [3, 3, 2, 1, 0], "ao_count_atom": 29, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 29)
    ...
    def test_reads_controlled_tail_damped_f_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "controlled-tail-damped-f.orb"
            orbital.write_text("orbital\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_tail_damped_f", "nu": [3, 3, 2, 1, 0], "ao_count_atom": 29, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 29)
    ...
    def test_reads_controlled_lowest_g_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "controlled-lowest-g.orb"
            orbital.write_text("orbital\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_lowest_g", "nu": [3, 3, 2, 0, 1], "ao_count_atom": 31, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 31)
            self.assertEqual(result["layout"], "3s3p2d1g")

    def test_reads_controlled_contracted_g_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "controlled-contracted-g.orb"
            orbital.write_text("orbital\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_contracted_g", "nu": [3, 3, 2, 0, 1], "ao_count_atom": 31, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 31)
    ...
    def test_reads_controlled_optimized_g_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "controlled-optimized-g.orb"
            orbital.write_text("orbital\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_optimized_g", "nu": [3, 3, 2, 0, 1], "ao_count_atom": 31, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 31)
    ...
    def test_reads_controlled_optimized_g_lowpass_candidate(self):
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            orbital = root / "C.orb"
            orbital.write_text("lowpass optimized g\n", encoding="ascii")
            digest = hashlib.sha256(orbital.read_bytes()).hexdigest()
            (root / "CANDIDATE.json").write_text(
                json.dumps({"status": "success", "profile": "controlled_optimized_g_lowpass", "nu": [3, 3, 2, 0, 1], "ao_count_atom": 31, "orbital_filename": orbital.name, "orbital_sha256": digest}),
                encoding="ascii",
            )
            result = read_candidate(root)
            self.assertEqual(result["nao_atom"], 31)
