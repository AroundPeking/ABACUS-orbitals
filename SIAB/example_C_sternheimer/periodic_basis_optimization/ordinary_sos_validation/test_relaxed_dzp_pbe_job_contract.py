...
class RelaxedDzpPbeJobContractTest(unittest.TestCase):
    def test_job_runs_dynamic_candidate_atom_and_solid_before_rpa(self):
        ...
        self.assertIn("ATOM_RESTART_MIXING_BETA", text)
        ...
        self.assertIn('set_input_key "$input" mixing_beta "$atom_restart_mixing_beta"', text)
        self.assertIn('set_input_key "$input" mixing_beta_mag "$atom_restart_mixing_beta"', text)
