...
grep -Fq 'profile_args=(--second-primitive-amplitude "$second_primitive_amplitude")' "$job"
grep -Fq 'F_DAMPING_RADIUS_BOHR' "$job"
grep -Fq 'F_DAMPING_POWER' "$job"
grep -Fq 'profile_args=(--damping-radius-bohr "$damping_radius_bohr" --damping-power "$damping_power")' "$job"
