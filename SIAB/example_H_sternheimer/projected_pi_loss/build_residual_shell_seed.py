#!/usr/bin/env python3
"""Append one leading atomic residual mode in a requested angular channel."""
...
def parse_nu(value):
    fields = value.split(",")
    if len(fields) != 5:
        raise argparse.ArgumentTypeError("expected-nu must contain five integers")
    try:
        result = tuple(int(field) for field in fields)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            "expected-nu must contain five integers"
        ) from exc
    if any(count < 0 for count in result):
        raise argparse.ArgumentTypeError("expected-nu entries must be nonnegative")
    return result
...
def parse_args():
    ...
    parser.add_argument("--angular-momentum", type=int, choices=range(5), required=True)
    parser.add_argument("--expected-nu", type=parse_nu, required=True)
...
def main():
    ...
    if leading_eigenvalue <= 0.0:
        raise RuntimeError("the leading residual eigenvalue is not positive")
    ...
    candidate["H"][angular_momentum] = torch.cat(
        (candidate["H"][angular_momentum], leading_mode), dim=1
    )
    expected_nu = list(args.expected_nu)
    expected_nu[angular_momentum] += 1
    if tuple(channel.shape[1] for channel in candidate["H"]) != tuple(expected_nu):
        raise RuntimeError("the residual seed changed an unexpected orbital channel")
    write_optimizer_coefficients(output_path, candidate)

    added_ao = 2 * angular_momentum + 1
    payload = {
        "format_version": 1,
        "source_family": "H",
        ...
        "l": angular_momentum,
        "radial_mode": args.expected_nu[angular_momentum],
        "added_ao": added_ao,
        "leading_eigenvalue": leading_eigenvalue,
        "gain_per_added_ao": leading_eigenvalue / added_ao,
